...
export default function OfferModal({
  ...
  const [approvedAmountDisplay, setApprovedAmountDisplay] = useState("");
  ...
  const handleConfirmApproved = () => {
    const num = Number(approvedAmountRaw);
    ...
    setApprovedAmountDisplay(formatRM(num));
  };
              ...
              <div className="flex items-center gap-3 mt-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // remove RM formatting since user is re-typing
                    const formatted = raw ? new Intl.NumberFormat("en-US").format(Number(raw)) : "";
                    setApprovedAmountDisplay(formatted);

                    setApprovedConfirmed(false); // un-confirm
                  }}
              ...
              <div>
                ...
                <div>
                  <Calendar
                    ...
                    onSelect={(d) => onSelectDate(d ?? undefined)}
                    hideNavigation={true}
                    month={displayMonth ?? undefined}
                    disabled={startOfToday()}
                    hidden={{ before: startOfToday() }}      // <-- alternative prop name some libs use
                  />
                </div>
